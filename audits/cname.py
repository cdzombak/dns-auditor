...
def audit(
    res: resolver.Resolver, verbose: bool, records: typing.Iterable[NormalizedRecord]
) -> bool:
    """
    ...
    for r in records:
        ...
        try:
            ...
            cprint(
                "[!] {recname:s}: CNAME {cname:s} does not resolve to an A record".format(
                    recname=r.name, cname=r.data
                ),
                "red",
            )
            retv = False
            ...
            cprint(
                "[!] {recname:s}: CNAME {cname:s} lookup timed out".format(
                    recname=r.name, cname=r.data
                ),
                "yellow",
            )
            ...
            cprint(
                "[!] {recname:s}: CNAME {cname:s} lookup failed: {exc:s}".format(
                    recname=r.name, cname=r.data, exc=str(e2)
                ),
                "red",
            )
            retv = False
