...
def audit(policy, verbose: bool, records: typing.List[Record]) -> bool:
    ...
    for r in records:
        ...
        parts = r.data.split(' ')
        if len(parts) != 3:
            cprint("[!] {n:s}: CAA record has wrong number of parts: {data:s}".format(
                n=r.name, data=r.data), 'red')
            retv = False
            continue

        flags = int(parts[0])
        if flags != 0 and flags != 128:
            cprint("[!] {n:s}: CAA record has invalid flags: {data:s}".format(
                n=r.name, data=r.data), 'yellow')
            retv = False
            continue

        tag = parts[1]
        if tag not in ('issue', 'issuewild', 'issuemail', 'issuevmc', 'iodef', 'contactemail', 'contactphone'):
            cprint("[!] {n:s}: CAA record has invalid tag: {data:s}".format(
                n=r.name, data=r.data), 'red')
            retv = False
            continue
        ...
        value = parts[2]
        if not value:
            cprint("[!] {n:s}: CAA record value is empty: {data:s}".format(
                n=r.name, data=r.data), 'red')
            retv = False
            continue
        ...
        if tag == 'iodef':
            if value.lower().startswith('mailto:'):
                ...
                if not validators.email(address):
                    cprint("[!] {n:s}: CAA iodef value is not a valid email address: {data:s}".format(
                        n=r.name, data=r.data), 'red')
                    retv = False
                    continue
            if value.lower().startswith('http'):
                if not validators.url(value):
                    cprint("[!] {n:s}: CAA iodef value is not a valid URL: {data:s}".format(
                        n=r.name, data=r.data), 'red')
                    retv = False
                    continue
        ...
        if tag == 'contactemail':
            if not validators.email(value):
                cprint("[!] {n:s}: CAA contactemail value is not a valid email address: {data:s}".format(
                    n=r.name, data=r.data), 'red')
                retv = False
                continue
