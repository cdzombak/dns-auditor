    ...
    for r in records:
        if r.type == 'MX':
            try:
                ...
                cprint("[!] {recname:s}: MX {recdat:s} does not resolve to an A record"
                       .format(recname=r.name, recdat=r.data), 'red')
                retv = False
                continue
            except resolver.LifetimeTimeout:
                cprint("[!] {recname:s}: MX {recdat:s} lookup timed out"
                       .format(recname=r.name, recdat=r.data), 'yellow')
                continue
            except dns.exception.DNSException as e2:
                cprint("[!] {recname:s}: MX {recdat:s} lookup failed: {exc:s}"
                       .format(recname=r.name, recdat=r.data, exc=str(e2)), 'red')
                retv = False
                continue
            if verbose:
                for a in ans:
            ...
            for pair in parts:
                ...
                if len(pair_parts) != 2:
                    cprint("[!] {recname:s}: DMARC {recdat:s} is invalid"
                           .format(recname=r.name, recdat=r.data), 'red')
                    retv = False
                    continue
                ...
                if tag == 'v':
                    if value != 'DMARC1':
                        cprint("[!] {recname:s}: DMARC version is invalid: {recdat:s}"
                               .format(recname=r.name, recdat=r.data), 'red')
                        retv = False
                        continue
                    has_v = True
                    ...
                    if value not in ('none', 'quarantine', 'reject'):
                        cprint("[!] {recname:s}: DMARC policy is invalid: {recdat:s}"
                               .format(recname=r.name, recdat=r.data), 'red')
                        retv = False
                        continue
                    has_p = True
                    ...
                    if not value.startswith('mailto:'):
                        cprint("[!] {recname:s}: DMARC rua is invalid: {recdat:s}"
                               .format(recname=r.name, recdat=r.data), 'red')
                        retv = False
                        continue
                    if not validators.email(value[7:]):
                        cprint("[!] {recname:s}: DMARC rua is not a valid email address: {recdat:s}"
                               .format(recname=r.name, recdat=r.data), 'red')
                        retv = False
                        continue
                    has_rua = True
            if has_v and has_p and has_rua:
                ...
                cprint("[!] {recname:s}: DMARC {recdat:s} is missing version tag"
                       .format(recname=r.name, recdat=r.data), 'red')
                retv = False
            elif not has_p:
                cprint("[!] {recname:s}: DMARC {recdat:s} is missing policy tag"
                       .format(recname=r.name, recdat=r.data), 'red')
                retv = False
            elif not has_rua:
                cprint("[!] {recname:s}: DMARC {recdat:s} is missing rua tag"
                       .format(recname=r.name, recdat=r.data), 'yellow')
                retv = False
    ...
    if policy.getboolean('RequireSPF'):
        for name in valid_mx_records.keys():
            if name not in has_spf_records:
                cprint("[!] {recname:s}: MX record exists, but no SPF record found"
                       .format(recname=name), 'red')
                retv = False
            elif verbose:
